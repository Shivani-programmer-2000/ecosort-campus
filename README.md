...
| | |
|---|---|
| **Author** | NADELLA SHIVANI, JAIN UNIVERSITY |
| **Primary SDG** | SDG 12: Responsible Consumption and Production |
| **Secondary SDGs** | SDG 11: Sustainable Cities and Communities · SDG 13: Climate Action |
| **AI components** | Keyword (TF-IDF) retrieval over a cited knowledge base, retrieval-only by default · IBM Bob (build time) · IBM Granite models and photo input (both optional, neither used in the offline demo; adding a Granite key turns the same pipeline into full Retrieval-Augmented Generation, RAG) |
...
* The Streamlit app (`app.py`) is styled to match the website: four bins that open, the same colours and sections. The page footer text comes from an `AUTHOR` setting, so you never edit code to put your name in.
...
Copy `.env.example` to `.env` (or `.streamlit/secrets.toml.example` to `.streamlit/secrets.toml`), then choose one provider. The secrets example now defaults to `offline`; switch it to `watsonx` only when you have real keys. No credits are needed for offline mode. See **[Do I need credits?](#5-do-i-need-credits-or-keys)** below.
...
> The watsonx.ai, Ollama and photo-mode code paths follow the vendors' documented APIs (the watsonx SDK signatures were checked against the installed package) but were **not run against live services** while this repository was written. Test them with your own account and update this note with what you tested. The offline mode, quota logic, tests and evaluation were run.
...
2. **Live app**: <https://share.streamlit.io> → Create app → this repo, branch `main`, file `app.py` → Advanced settings → Secrets: `LLM_PROVIDER = "offline"` and `AUTHOR = "Your Name, Your College"`.
...
│   ├── privacy.py              # strips e-mails, phone numbers and PAN numbers
...
├── eval/                       # test items, extra items written with Bob, scoring script
├── tests/                      # pytest suite
├── docs/                       # website, architecture, responsible AI, Bob workflow, Bob security review, staff user guide, slide deck
├── AGENTS.md · .bob/           # project rules Bob generated with /init
...
**IBM Bob** is used as the AI development partner at *build time*. At *run time*, users interact with a retrieval-based assistant that answers from the knowledge base. **IBM Granite** can be switched on to write the answers, but the submitted demo runs in offline mode without it. A human reviews everything Bob produces.

What Bob produced in this project, and what the human review changed:

| Bob task | Output | Human review |
|---|---|---|
| `/init` | [`AGENTS.md`](AGENTS.md) and `.bob/` rules for Plan, Ask and Agent modes | Checked against the real commands and architecture |
| Security review | [`docs/bob_review.md`](docs/bob_review.md): 10 findings (3 medium, 4 low, 3 informational, none high) | Tested each claim against the code. Three were wrong or overstated: Aadhaar numbers were already stripped by the phone pattern, the stream guardrail *can* be influenced by the user's own words, and one recommended change was not needed |
| Extra test items | [`eval/bob_extra_items.csv`](eval/bob_extra_items.csv): 20 items, 18 correct, 0 confidently wrong | The 2 misses (`mango seed`, `pencil`) are knowledge-base gaps. The misspelled items pass only because another word in the query matches |
| User guide | [`docs/user_guide.md`](docs/user_guide.md), one page for housekeeping staff | Corrected the bin colours (the app shows orange for Sanitary, not red), removed a wrong "works offline" claim, removed physical bin colours the knowledge base does not define |
| Security fixes | Error text hidden from the UI (full detail logged), API key and project ID hidden from `repr()`, model-reply cache capped at 256 entries, PAN numbers stripped, 6 new tests | Rejected a first version of one test that also passed on the unfixed code. It was rewritten so it fails on the old code |
...
| **Privacy** | No accounts. E-mails, phone numbers, PAN numbers and other long digit strings (which covers 12-digit Aadhaar numbers) are stripped. Names and addresses are **not** detected. Photos stay in memory. Feedback is three integer counters. |

### Security review

A build-time review ([`docs/bob_review.md`](docs/bob_review.md)) found no hard-coded secrets and no high-severity issues. Fixed and covered by tests in `tests/test_security.py` and `tests/test_privacy.py`: exception text no longer reaches the page, credentials no longer appear in `repr(Settings)`, the reply cache is capped, and PAN numbers are stripped.

Still open:

* **Prompt injection (only when an LLM is enabled).** The typed item and the knowledge-base text go into the prompt as plain text. The guardrail rejects any stream the model returns that was not in the retrieved evidence, but retrieval itself uses the user's words, so a user can influence their own answer. Offline mode has no model to inject into. Wrap the item in delimiters and validate the reply format before enabling Granite for the public.
* **Data residency.** `WATSONX_URL` defaults to the `us-south` region. Set it to the region you are allowed to use before enabling watsonx.
...
`eval/run_eval.py` scores only `eval/test_items.csv`. A second set of 20 items written with Bob, [`eval/bob_extra_items.csv`](eval/bob_extra_items.csv), scored **18/20 with 0 confidently wrong** when run by hand; the two misses return "Not sure".
...
* Keyword retrieval has no fuzzy matching. A misspelled word on its own (for example `baterry`) returns "Not sure", which fails safe. A misspelling next to a correctly spelled word (`banan peel`) still works. Slang and non-English item names are not handled yet.
* Personal data typed by mistake is only partly removed: e-mails, phone numbers, PAN and long digit strings are stripped, but names, addresses and room numbers are not. The page tells users not to type them.
* When an LLM is enabled, replies are cached in memory (up to 256 entries) until the app restarts. Nothing is written to disk.
* The default offline mode uses **no language model**: it returns the best-matching knowledge-base passage. Model-written answers need IBM Granite, which is supported but was not run against a live service for this submission.
...
- [ ] Prompt hardening for LLM mode: delimit the user item and validate the reply format
