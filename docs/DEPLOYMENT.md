...
| **Streamlit app** (`app.py`) | Streamlit Community Cloud | Free | Works offline with no keys; add Granite keys only if you want model-written answers |
...
4. Click **Advanced settings**:
   ...
   * **Secrets**: for offline mode (no keys), paste these two lines with your own details:
     ```
     LLM_PROVIDER = "offline"
     AUTHOR = "Your Name, Your College"
     ```
     `AUTHOR` is shown in the page footer. To use Granite instead, paste your full `secrets.toml` from step 3.
